...
# fallback is tomcat7
if [[ $( apt-cache show tomcat10 2> /dev/null | egrep "Version: 9" | wc -l ) -gt 0 ]]; then
...
if [ -x "$( command -v ufw )" ]; then
    ...
    if [[ $(ufw status | grep inactive | wc -l) -eq 0 ]]; then
        # Check if 5000 is not already allowed
        if [[ $(ufw status | grep "5000/tcp" | grep "ALLOW" | grep "Anywhere" | wc -l) -eq 0 ]]; then
            # ufw is running, but 5000 is not allowed, add it
            ufw allow 5000/tcp comment 'allow tomcat'
        fi
    fi
fi    
...
if [ $? -eq 0 ]; then
    ...
    if [[ $(iptables --list-rules | grep -- "-p tcp" | grep -- "--dport 5000" | grep -- "-j ACCEPT" | wc -l) -eq 0 ]]; then
        # ALlow it
        iptables -A INPUT -p tcp --dport 5000 --jump ACCEPT
    fi
fi
